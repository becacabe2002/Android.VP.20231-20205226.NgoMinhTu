package com.mtu.data_presistance_demo
...
import com.mtu.data_presistance_demo.databinding.ActivityMainBinding
import java.io.File
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonReadResource.setOnClickListener {
            val inputStream = resources.openRawResource(R.raw.resource)
        ...
        binding.buttonWriteExternal.setOnClickListener {
            val filePath = getExternalFilesDir(null)?.absolutePath + "/my_data.txt"
        ...
        // list all records
        val cs = db.rawQuery("select * from tblAMIGO order by recID desc limit 5", null)
    ...
    fun createTable() {
        db.beginTransaction()
        try {
            val cursor = db.rawQuery("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='tblAMIGO'", null)
            // if db is there is no tblAMIGO
